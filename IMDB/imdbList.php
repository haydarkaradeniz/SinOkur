...
<html lang="en">
  <head>
    ...
    <link type="text/css" rel="stylesheet" href="../imdb/css/styleList.css"/>
  ...
  <body>
    ...
    <div class="header-seperator" id="sineokur-mainRow">
    </div>
    <div class="user-list-main-div">
      <div class="user-list-header-div">
        <div v-if="!profile.guestMode" class="create-list pointer" @click="createList()">
          YENİ LİSTE YARAT
        </div>
        <div v-for="(listData, index) in userListInfo" class="list-name-item" :style="{'margin-bottom':index==7?'15px':'0px'}">
            <i class="fa fa-check list-check" :style="{color:index>7?'green':'red', 'padding-right':'5px'}"></i><span @click="selectList(index)" class="list-name" :class="{'selected-list':index == selectedListIndex}">{{listData.header + ' (' + listData.count + ')'}}</span>
        </div>
      </div>
      <div :style="{'width':(mainRowWidth>665+(userListDataInfo.length>0 && userListDataInfo[0].imdbType == 'person'?0:0)?mainRowWidth-300:365+(userListDataInfo.length>0 && userListDataInfo[0].imdbType == 'person'?0:0))+'px', 'min-width':365+(userListDataInfo.length>0 && userListDataInfo[0].imdbType == 'person'?0:0)+'px'}">
          <div>
            <span class="selected-list-title">{{userListInfo[selectedListIndex].header}}</span>
            <span :style="{'position':'absolute', 'left':(mainRowWidth+screenLeftOffset-110)+'px', 'width':'130px'}">
          ...
          <div class="header-sort" v-if="userListDataInfo.length>0">
          ...
          <div class="flex-wrap" style="margin-top: 50px;">
            <template v-for="(data, index) in userListDataInfo">
              <div v-if="data.imdbType == 'person'" class="person-card">
                <div class="person-poster-div">
                  ...
                  <img class="person-remove-list pointer" :src="getBaseUrl()+'images/cikar.svg'" @click="removeFromUserList(data.listId, data.imdbId)" v-if="!profile.guestMode">
                </div>
              ...
              <div v-if="data.imdbType == 'movie'" class="movie-card">
                <div class="movie-poster-div">
                  <a :href="subPath+'/app.php/filmler?detail=1&id='+data.imdbId" target="_parent"><img :src="data.poster" class="movie-poster"/></a>
                  <img v-if="data.userRating" class="movie-score-img" :src="getBaseUrl()+'images/' + parseInt(data.userRating) + '.svg'">
                  <img class="movie-remove-list pointer" :src="getBaseUrl()+'images/cikar.svg'" @click="removeFromUserList(data.listId, data.imdbId)" v-if="!profile.guestMode">
                </div>
    ...
    <script src="../imdb/scripts/app_list.js"></script>
